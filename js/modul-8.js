...
// Деструктуризація :  const {title} = book, те саме що:  const title = book.title    (ДЕСТРУКТУРИЗАЦІЯ - СТВОРЮЄМО ЗМІННУ "title" і присвоюємо їй значення "The Last Kingdom" однойменної властивості об'єкта "book.title")
...
//                  ЧАСТКОВА деструктуризація (дивись рядок 927):

// Іноді з об'єкту необхідно деструктуризувати тільки перші N властивостей, а інші властивості зберегти у новий об'єкт.
// Деструктуризуючи об'єкт, можна розпакувати перші необхідні властивості і присвоїти іншу частину властивостей об'єкта змінній, використовуючи операцію "...rest" :

// ПРИКЛАД для розуміння :
// Операція деструктуризації з rest оператором (...) витягує властивості "firstName" та "lastName" з об'єкта "user", а всі інші властивості, що залишилися, поміщаються в НОВИЙ об'єкт "newUser", при цьому оригінальний об'єкт "user" НЕ ЗМІНЮЄТЬСЯ :
...
//  ЧАСТКОВА деструктуризація ОБ'ЄКТІВ (дивись рядок 846):
...
// УРОК-2  Mодуль-8. Події та бібліотеки (КАХУТ - 00:20:31) :

// ПОЯСНЕННЯ-19 :   Что будет выведено в консоль?
...
// ПОЯСНЕННЯ :
// Функція "foo" використовує деструктуризацію об'єкта у параметрі: { username }.
// Це означає, що вона очікує об'єкт із властивістю "username", наприклад:  foo({ username: 'Jacob' }).
// Але при виклику передається об'єкт { user: 'Jacob' } — у ньому немає властивості username.
// Тому username отримує значення "undefined", яке й виводиться в консоль.
...
// УРОК-2  Mодуль-8. Події та бібліотеки (КАХУТ - 00:21:53) :

// ПОЯСНЕННЯ-20 :   Что будет выведено в консоль?

// function foo(params = {}) {
//     console.log(params);           //   { name: 'Jacob' }
// }

// foo({ name: 'Jacob' });


// ПОЯСНЕННЯ  - у коді НЕМАЄ деструктуризації :
// У функції foo параметр "params" має значення за замовчуванням "{}" (порожній об'єкт). "params" — це звичайний параметр функції (не деструктуризація).

// params = {} — це значення за замовчуванням, яке використовується, якщо аргумент не передано або передано undefined.

// У цьому випадку під час виклику foo({ name: 'Jacob' }) передається реальний аргумент — об'єкт { name: 'Jacob' }.

// Тому значення за замовчуванням ігнорується, і в консоль виводиться саме переданий об'єкт.
...
// УРОК-2  Mодуль-8. Події та бібліотеки (КАХУТ - 00:22:36) :

// ПОЯСНЕННЯ-21 :   Что будет выведено в консоль?

// function foo({ username }) {
//     console.log(username);    //   помилка   Uncaught TypeError:...
// }
...
// Це означає, що при виклику функції в неї має бути переданий об'єкт, наприклад foo({ username: 'Jacob' }).

// Але у цьому випадку виклик відбувається без аргументів: foo().

// Оскільки функція намагається деструктуризувати "undefined" (тому що параметр не передано), виникає ПОМИЛКА: при деструктуризації "undefined" або "null" завжди виникає помилка в JavaScript.

// Якщо потрібно УНИКНУТИ ПОМИЛКИ, можна задати значення за замовчуванням (КАХУТ - 00:23:10) :
...
// // ПОЯСНЕННЯ :
    // Коли викликаємо foo() без аргументів, параметр отримує значення за замовчуванням "{}".

    // Далі відбувається деструктуризація { username } з порожнього об'єкта "{}".

    // Властивість "username" не знайдено → отримуємо "undefined".

    // ПОМИЛКИ НЕМАЄ, тому що деструктуризується об'єкт, а не "undefined".
...
// УРОК-2  Mодуль-8. Події та бібліотеки :

// ПОЯСНЕННЯ-22 :   Що буде выведено в консоль "undefined" чи "помилка" ?
// !!!  Пояснення різниці між undefined (якщо змінна ІСНУЄ) та ReferenceError (помилка посилання), якщо НЕ ІСНУЄ.
...
// -  ДЕСТРУКТУРИЗАЦІЯ: { name, age, city } - СТВОРЮЄМО ЗМІННІ (name, age, city) і присвоюємо їм значення однойменних властивостей об'єкта
// -  В об'єкті "user" немає властивості "city", тому змінна "city" створюється, але отримує значення "undefined".
// -  Потім виклик console.log(city) — змінна ІСНУЄ, виводиться "undefined".
...
// const { name, age, city: userCity } = user;      //   ДЕСТРУКТУРИЗАЦІЯ - СТВОРЮЄМО ЗМІННІ (name, age, userCity) і присвоюємо їм значення однойменних властивостей об'єкта. 
// Створюються змінні:
// const name = "Alice"
// const age = 30
// const userCity = "Kyiv"

// В змінну "userCity" присвоюємо значення властивості "city" об'єкта "user".
// ТОБТО в ЗМІННУ "userCity" присвоюємо значення "Kyiv" ВЛАСТИВОСТІ об'єкта "city". 
// "city" залишається назвою властивості об'єкта, а не змінною.

// !!!  ТОБТО в запису { name, age, city: userCity } city - це не змінна, а властивость об'єкта "user". 
// console.log(city);  //  помилка    ReferenceError (помилка посилання)
...
// !!!  ТОБТО :
// -  в першому випадку помилки немає, тому що ЗМІННА ІСНУЄ (навіть з undefined), 
// -  в другому — ЗМІННОЇ просто НЕ ІСНУЄ.
...
// УРОК-2 Mодуль-8. Події та бібліотеки (00:45:30) :

// ПОЯСНЕННЯ-23 :   ГЛИБОКА ДЕСТРУКТУРИЗАЦІЯ (дивись рядок 755) :
...
// const { skills } = user;           //   {html: true, css: true, js: false}

// ........

// ВАРІАНТ-1  ГЛИБОКА ДЕСТРУКТУРИЗАЦІЯ :
// const { css, html, js } = skills;
...
// ВАРІАНТ-2  ГЛИБОКА ДЕСТРУКТУРИЗАЦІЯ :
// const { skills: {css, html, js} } = user;
...
// УРОК-2 Mодуль-8. Події та бібліотеки (00:49:20) :

// ПОЯСНЕННЯ-24 :   ДЕСТРУКТУРИЗАЦІЯ МАСИВА (дивись рядок 873) :
...
// // ВАРІАНТ-2  (ДЕСТРУКТУРИЗАЦІЯ - порядок має значення!!!) :
...
// // ВАРІАНТ-3  (ДЕСТРУКТУРИЗАЦІЯ - якщо потрібен тільки елемент під [0] індексом  і  елемент під [2] індексом. Для цього під час ДЕСТРУКТУРИЗАЦІЇ після першого елемента ставимо пробіл і кому, або можна без пробілу - тоді читабільність буде гірше) :

// const [first, , third] = arr;      //   або  const [first,, third] = arr;
// console.log(first, third);    //   10 12

// .........

// // ВАРІАНТ-4  (ДЕСТРУКТУРИЗАЦІЯ - якщо в масиву в нас 5-ть елементів, а ДЕСТРУКТУРИЗАЦІЇ вказав 7-м елементів) (00:53:50) :

// const [el1, , el3, , , , el7] = arr;     
// console.log(el7);    //   undefined

// Щоб не було "undefined", використовуємо значення за замовчуванням  (дивись рядок 894) :

// const [el1, , el3, , , , el7 = 20] = arr;     
// console.log(el7);    //   20


// .........

// // ВАРІАНТ-5  (ДЕСТРУКТУРИЗАЦІЯ - можна використати "...rest" оператор) (00:55:00) (дивись рядок 910) :

// const [el1, el3, ...rest] = arr;     
// console.log(el1, el3, rest);    //   10 11 [12, 13, 14]
...
// ПОЯСНЕННЯ-26 :   ДЕСТРУКТУРИЗАЦІЯ  ПАРАМЕТРІВ з "undefined" :
...
// УРОК-2 Mодуль-8. Події та бібліотеки ( 01:28:00) :

// ПОЯСНЕННЯ-27 :   ДЕСТРУКТУРИЗАЦІЯ  ПАРАМЕТРІВ в ЦИКЛАХ :
...
// УРОК-1 Mодуль-8. Події та бібліотеки ( 00:49:00) :