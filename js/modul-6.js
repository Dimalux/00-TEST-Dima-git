...
// !!!  ВАЖЛИВО : Якщо оголошено ПУБЛІЧНУ властивість "email" (замість приватної "#email" буде "email"), геттер НЕ працює !!! 
...
// !!!  ВАЖЛИВО :   Якщо змінити ІМ'Я геттера і сеттера, наприклад, на :
...
// Коли ми пишемо в зовнішньому коді "user.email" або "user.test" ми звертаємость до властивості типу accessor property. Accessor property — це ВЛАСТИВІСТЬ об'єкта (або прототипу) з певним іменем (email, test), яка замість зберігання значення містить геттер і/або сеттер.

// "user.email" і "user.test" — це доступ до accessor property, який рушій JS автоматично перетворює на виклик геттера (при читанні) або сеттера (при присвоєнні). Просто синтаксис приховує це. Тому й кажуть, що геттер і сеттер "ІМІТУЮТЬ публічну властивість" — зовні "user.email" і "user.test" виглядає як читання поля, а технічно "email" або "test" — це accessor property з геттером і/або сеттером"
