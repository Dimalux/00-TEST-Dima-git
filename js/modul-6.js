...
// Тобто різниця між call і apply полягає лише у формі передачі аргументів. Вибір між call і apply залежить від того, як саме ми хочемо передавати аргументи до функції в конкретній ситуації. У всіх інших аспектах вони працюють ідентично: викликають функцію в контексті зазначеного об'єкта і передають їй аргументи.
...
// Геттери і сеттери — це спеціальний синтаксис оголошення методів для взаємодії з властивостями. Геттер і сеттер імітують звичайну публічну властивість класу, дозволяють взаємодіяти з іншими властивостями зручнішим способом.

// !!!  ВАЖЛИВО : Якщо оголошено ПУБЛІЧНУ властивість "email" (замість приватної "#email" буде "email"), геттер і сеттер НЕ працює !!! 

// Публічна властивість (this.email = ...) створює власну властивість об'єкта.
// Геттер/сеттер — це методи, які не створюють властивість, а ПЕРЕХОПЛЮЮТЬ спроби доступу до НЕІСНУЮЧОЇ властивості з такою назвою.

// Якщо оголосити і публічну властивість і геттер/сеттер (публічна властивість "ПЕРЕКРИВАЄ" геттер/сеттер, публічна властивість має вищий пріоритет. Геттер/сеттер стають мертвим кодом — вони є, але ніколи не викликаються!). Геттер/сеттер працюють тільки коли НЕМАЄ публічної властивості з такою самою назвою. Саме тому їх зазвичай використовують разом із приватними властивостями (#), щоб уникнути конфлікту. Конфлікт виникає, коли публічна властивість і геттер/сеттер мають ОДНАКОВУ назву. У такому разі публічна властивість "перемагає", і геттер/сеттер просто НЕ викликаються. ЩОБ УНИКНУТИ КОНФЛІКТУ використовувати приватну властивість з іншою назвою (або ту саму, але з #), щоб публічної властивості з такою назвою не існувало.

// РОЗ'ЯСНЕННЯ-1 (НЕ для нашого випадку):  НЕ можна одночасно мати обидві властивості з одним іменем на одному об'єкті. Це не конфлікт двох сутностей, які співіснують, — це одна комірка в об'єкті (оголошена властивість "email" займає одну комірку, НЕ може бути в пам'яті 2-і комірки з однаковим ім'ям), яка може бути або data, або accessor. Тобто властивість з іменем "email" в об'єкті може бути лише одна. Або data (публічна властивість), або access (accessor-властивість).

// Якщо оголосити і data, і accessor на тому самому об'єкті (наприклад, у літералі), то перемагає останнє оголошення. «У літералі» — це скорочення від «у літералі об'єкта» (object literal). Тобто коли об'єкт створюється не через class і не через new, а безпосередньо через фігурні дужки { ... } :

// const user = {
//   book: 5,
//   get book() { return 7; }   // перезаписує попереднє data-поле
// };
// user.book;   // 7 — геттер ПЕРЕМІГ, бо оголошений пізніше

// ........

// const user2 = {
//   get book() { return 7; },
//   book: 5                      // перезаписує accessor
// };
// user2.book;  // 5 — data ПЕРЕМОГЛА

// Тут це справді одна комірка, яку ПЕРЕЗАПИСУЮТЬ. Порядок має значення !!!

// ........

// РОЗ'ЯСНЕННЯ-2 (ДЛЯ нашого випадку): ми маємо КЛАС і екземпляр класу, де справді є два рівні:

// class User {
//   book = 5;                    // на екземплярі
//   get book() { return 7; }     // на User.prototype
// }
// І тут механізм інший — НЕ «ПЕРЕЗАПИС комірки», а ЗАТІНЕННЯ:

// book = 5        //   власна властивість екземпляра;

// get book()      //   властивість прототипу;

// ПОШУК u.book знаходить власну властивість екземпляра раніше - прототип НЕ ДОСЯГАЄТЬСЯ !!!

// ........

// ВИСНОВОК : 
// - якщо в нас ЛІТЕРАЛ - усе на одному об'єкті (на одному рівні) - то буде ПЕРЕЗАПИС;
// - якщо в нас КЛАС - data на екземплярі, accessor на прототипі - то буде ЗАТІНЕННЯ.
...
// Геттери і сеттери це значення властивості типу accessor property. Accessor property (accessor-властивість) — це ВЛАСТИВІСТЬ об'єкта (або прототипу) з певним іменем (наприклад "email"), яка замість зберігання значення містить геттер і/або сеттер. Коли ми оголошуємо геттер/сеттер з тим самим ім'ям, що й властивість, ми створюємо accessor property (accessor-властивість) — властивість об'єкта, яка «виглядає» як звичайна властивість для зовнішнього коду, але всередині виконує логіку.

// !!!  ВАЖЛИВО :   Якщо змінити ІМ'Я геттера і сеттера, наприклад, з :
...
// то зміниться ІМ'Я властивості типу accessor property, через яку зовнішній код взаємодіє з приватним значенням "#email". Тобто замість "email" у зовнішньому коді тепер буде "test". Ім'я "test" — це лише публічна "обгортка". Внутрішня властивість усе одно "#email". 

// Властивості типу accessor property на ІМ'Я "email" тепер немає — якщо спробуємо звернутися до "user.email", отримуємо undefined, хоча "#email" усередині класу залишилась.

// "user.email" або "user.test" — це доступ до властивості accessor property на ІМ'Я "email" або "test", який рушій JS АВТОМАТИЧНО (тобто без жодного синтаксису виклику з боку того, хто пише "user.email" або "user.test". Ніяких (), ніяких .get()) перетворює на виклик геттера (при читанні) або сеттера (при присвоєнні). Тобто "email" або "test" — це властивість (НЕ та, що зберігає значення) яка містить пару функцій (геттер і/або сеттер).
// Просто синтаксис приховує це. Тому й кажуть, що геттер і сеттер "ІМІТУЮТЬ публічну властивість" — зовні "user.email" і "user.test" виглядає як читання поля, а технічно "email" або "test" — це accessor property з геттером і/або сеттером".
...
// ПРОТОТИПНЕ  НАСЛІДУВАННЯ  в JavaScript.  Є 2-а ВАРІАНТИ створювання ПРОТОТИПІВ - ручне (Object.create()) і автоматичне (класи та new) :


// 1. ВАРІАНТ РУЧНОГО створювання ПРОТОТИПУ :
 
// Object.create() — явне створення прототипного зв'язку. Тут ми вручну вказуємо, що "animal" буде прототипом для "dog" :
...
// Класи та new — автоматичне встановлення прототипу
...
// 1) Створює новий порожній об'єкт "mango".
// 2) Встановлює його прототип як User.prototype.
// 3) Викликає конструктор з this, прив'язаним до нового об'єкта.
// 4) Повертає цей об'єкт "mango".

// //............

// ГОЛОВНА  ІДЕЯ  - Прототип — це об'єкт, на який посилається [[Prototype]], це приховане посилання на інший об'єкт. Цей інший об'єкт називається прототипом.
// Коли ми звертаємося до властивості, якої немає в об'єкті, JS шукає її в прототипі (і далі по ланцюжку).

// Порівняння підходів :

// Object.create()	                    Класи + new
// Ручне встановлення прототипу.       	Автоматичне (прототип = Class.prototype)
// Підходить для простих об'єктів.	    Підходить для складних структур з методами
// Немає синтаксичного цукру.	        Має "синтаксичний цукор" - конструктор, методи, super, static
// Прототип — будь-який об'єкт.	        Прототип — Class.prototype, який містить методи (методи класу додаються до спеціального об'єкта, який зберігається у властивості prototype самого класу - console.log(User.prototype); // {constructor: ƒ, getEmail: ƒ, changeEmail: ƒ}).

// ПРИМІТКА :
// Синтаксичний цукор (англ. syntactic sugar) — це синтаксичні конструкції в мові програмування, які не додають нової функціональності, а лише роблять код зручнішим, читабельнішим і приємнішим для написання.
// Синтаксичний цукор — це спосіб зробити код красивішим і зрозумілішим, не змінюючи того, що відбувається "під капотом". Класи в JavaScript — ідеальний приклад: вони роблять прототипне наслідування доступним для розуміння. 
...
// // Метод padEnd(str) — отримує параметр str (рядок) і додає його в кінець значення приватної властивості value об'єкта, який викликає цей метод.
// padEnd(str) {
// this.#value = this.#value + str;
// // this.#value = `${this.#value}${str}`;
// }

// // Метод padStart(str) — отримує параметр str (рядок) і додає його на початок значення приватної властивості value об'єкта, який викликає цей метод.
// padStart(str) {
//     this.#value = str + this.#value;
//     // this.#value = `${str}${this.#value}`;
// }

// // Метод padBoth(str) — отримує параметр str (рядок) і додає його на початок і в кінець значення приватної властивості value об'єкта, який викликає цей метод.
// padBoth(str) {
